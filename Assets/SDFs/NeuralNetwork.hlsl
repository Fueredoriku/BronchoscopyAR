...
StructuredBuffer<float4x4> _WeightMats;
StructuredBuffer<float4> _WeightVecs;
...
{
    // each bias is a vec4
    return _WeightVecs[off + chunk];
}

float4x4 LoadMat_float(int off, int matIdx)
{
    // each matIdx is a 4×4 block
    return _WeightMats[off + matIdx];
}
...
float model_occ_float(float3 p, out float result)
{
    ...
    float4 f[64]; // max size; use _Chunks in loops
    ...
    // --- HIDDEN LAYERS ---
    for (int layer = 1; layer < _Depth - 1; ++layer)
    {
        float4 nextF[64];
        int wOff = _OffHW[layer - 1];
        int bOff = _OffHB[layer - 1];
        for (int c = 0; c < _Chunks; ++c)
        {
            float4 acc = float4(0, 0, 0, 0);
            // multiply the (c, d) block of the weight mat against f[d]
    ...
    // --- OUTPUT LAYER ---
    // output is a single logit: sum over dot( row4, f[c] )
    for (int c = 0; c < _Chunks; ++c)
    {
