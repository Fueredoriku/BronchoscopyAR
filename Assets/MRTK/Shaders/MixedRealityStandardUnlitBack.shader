...
Shader "Mixed Reality Toolkit/StandardUnlitBack"
{
    Properties
    {
        // Main maps.
...
#if defined(_DIRECTIONAL_LIGHT)
                float inside = facing * 0.5 + 0.5;
#if defined(_RENDER_PIPELINE)
                float4 directionalLightDirection = _MainLightPosition;
#else
                float4 directionalLightDirection = _WorldSpaceLightPos0;
#endif
                fixed diffuse = max(0.0, dot(worldNormal, directionalLightDirection)) * inside + (1.0 - inside);
#if defined(_SPECULAR_HIGHLIGHTS)
                fixed halfVector = max(0.0, dot(worldNormal, normalize(directionalLightDirection + worldViewDir)));
                fixed specular = saturate(pow(halfVector, _Shininess * pow(_Smoothness, 4.0)) * (_Smoothness * 2.0) * _Metallic)*inside;
#else
                fixed specular = 0.0;
#endif
#endif
